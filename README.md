...
- STM32-ready UART equipment-status protocol for Espresso, Brew, and Frozen stations
- Externally reported `BUSY` stations are excluded from live recommendations until re-enabled
...
Tracks the availability and busy duration of preparation stations. It also accepts externally reported station state: a reported `BUSY` state persists until a later `AVAILABLE` update arrives.

### `EquipmentStatusParser`

Validates STM32 UART status lines and applies them to `EquipmentManager` safely. The parser accepts only the MVP station set (`ESPRESSO`, `BREW`, and `FROZEN`) and rejects malformed, unknown, or unsupported messages without changing scheduler state. See [the UART protocol](docs/uart-protocol.md).
...
1. Filters to waiting orders whose required station is available.
2. Selects the highest-urgency order as the anchor.
...
For live operation, the scheduler can receive station updates from the STM32 using this line format:
...
When Espresso is reported `BUSY`, espresso orders are not eligible for the next recommendation; eligible Brew, Frozen, or Other work can still be selected. Invalid lines leave equipment state unchanged.
...
### Compile the Equipment-Status Parser Tests

```powershell
g++ -std=c++17 -Wall -Wextra -Wpedantic test_equipment_status_parser.cpp EquipmentStatusParser.cpp EquipmentManager.cpp -o equipment_status_parser_tests.exe
```

### Compile the Simulation-Comparison Regression Test

```powershell
g++ -std=c++17 -Wall -Wextra -Wpedantic test_simulation_comparison.cpp SimulationRunner.cpp OrderGenerator.cpp Order.cpp Scheduler.cpp EquipmentManager.cpp -o simulation_comparison_tests.exe
```
...
```powershell
.\scheduler_tests.exe
.\equipment_status_parser_tests.exe
.\simulation_comparison_tests.exe
```
...
## Corrected Fixed-Seed Simulation Results

The FIFO and dynamic strategies now have equal station-level parallelism: each can start one operation on every free station during a simulation tick. Dynamic batching creates a separate batch for each free station, rather than limiting itself to one batch per tick.

Fixed-seed rush scenario: seed 42, 20 orders, 10 seconds per simulation tick.

| Metric | FIFO | Dynamic batching | Change |
| --- | ---: | ---: | ---: |
| Average wait | 18.25 ticks | 6.70 ticks | 63.3% lower |
| Maximum wait | 63 ticks | 32 ticks | 31 fewer ticks |
| Total simulation time | 83 ticks | 48 ticks | 35 fewer ticks |
| Equipment operations | 20 | 10 | 50.0% fewer |
| Peak concurrent operations | 3 | 3 | Equal |

Both strategies completed all 20 orders. Under the current simulation and batching-duration model, dynamic batching reduced average simulated wait time by 63.3% and modeled equipment operations by 50.0%. These are simulation results, not measurements from a live store.
